// Interface translations only. Original records and model outputs stay unchanged.
window.CLASS_TRANSLATIONS = {
  en: {
    cases: {
      "eval:PH_2016.06.08_Economy-Final.pdf:0": {
        ...
        review: {
          ...
          notes: ["Do not treat the model's explanation as a manually verified answer."],
          rankingFinding: null,
          baseFindings: [],
          sftFindings: [],
      ...
      "eval:05-03-18-political-release.pdf:5": {
        ...
        review: {
          ...
          notes: ["The question says 2019, but the survey footnote on the candidate pages explicitly says 2018. This is a mismatch between the source question and its material. Keep the original question and do not present the 2018 figure of 18% as a verified answer for 2019."],
          ...
          scope: "The existing spot check covered only two questions, the Base and SFT explanations, and their 10 candidate instances (6 distinct original images). It did not check gold pages missed by retrieval or estimate factual accuracy across all 1,658 questions."
        }
      },
    ...
    glossary: [
      {term: "RAG", en: "Retrieval-Augmented Generation", definition: "Retrieve useful material, then give it to a model to answer the question.", example: "Find the relevant report page before generating an answer."},
      {term: "Retrieval", en: "ColQwen2 in this project", definition: "Select a small set of candidate pages by their similarity to the question.", example: "Score the available pages and keep the top 5."},
      {term: "Reranking", en: "Reorder the candidates", definition: "Compare the retrieved candidates again to put more useful pages first.", example: "[A,B,C] becomes [B,A,C]; a missed page F cannot enter this list."},
      {term: "Candidate index", en: "Local page number", definition: "A page's 1-based position in the model's input image list.", example: "An output of 3 means the third input image, not necessarily PDF page 3."},
      {term: "Gold pages", en: "Positive / relevant-page labels", definition: "All pages labeled relevant in the dataset, including pages that retrieval misses.", example: "If gold={B,F}, the Recall denominator stays 2 even when F is absent."},
      {term: "Prompt", en: "Model input", definition: "The instructions, question, and content the model sees before generating its output.", example: "In this project: a question, five page images, and the required output format."},
      {term: "Completion", en: "Target assistant output", definition: "The assistant response the model learns to generate.", example: "A brief evidence explanation followed by the complete candidate order."},
      {term: "Token", en: "Sequence unit", definition: "One unit in the sequence processed by a model; a text token need not be a whole word.", example: "Training predicts the target completion one token at a time."},
      {term: "SFT", en: "Supervised Fine-Tuning", definition: "Further train an existing model using examples of inputs and target outputs.", example: "Our saved run uses 7,200 evidence-and-ranking demonstrations."},
      {term: "Loss", en: "Training error signal", definition: "A measure of disagreement between predictions and targets that training tries to reduce.", example: "Giving the correct next token a higher probability lowers its cross-entropy loss."},
      {term: "LoRA", en: "Low-Rank Adaptation", definition: "Fine-tune small added matrices while freezing the large base weight matrices.", example: "This SFT run uses rank 8 and alpha 32."},
      {term: "GRPO", en: "Group Relative Policy Optimization", definition: "Update a model by comparing rewards for several outputs generated for the same question.", example: "Increase the probability of outputs that do better within their group."},
      {term: "Reward", en: "Output score", definition: "A score computed by the training program to express a preference among outputs.", example: "A reward may score valid format and ranking without checking factual explanations."},
      {term: "Advantage", en: "Relative reward signal", definition: "How an output's reward compares with its group's average, often normalized.", example: "If every output has the same reward, their relative reward advantages are zero."},
      {term: "Recall@K", en: "Recall at K", definition: "The number of gold pages in the first K positions divided by the total number of gold pages.", example: "Two gold pages, with one ranked first: Recall@1 = 1/2."},
      {term: "Fallback", en: "Keep retrieval order", definition: "Use the original retrieval order when model output fails the complete-permutation checks.", example: "[2,2,3] repeats an index and is not a valid order for three pages."}
    ]
  },
  zh: {
    glossary: [
      {term: "RAG", en: "Retrieval-Augmented Generation", definition: "检索增强生成：先找有用资料，再交给模型回答。", example: "先找到相关报表页，再让模型根据该页回答问题。"},
      {term: "检索", en: "Retrieval", definition: "按页面与问题的相似程度，选出少量候选页。", example: "本项目用 ColQwen2 给页面打分，再取前 5 页。"},
      {term: "重排序", en: "Reranking", definition: "重新比较检索得到的候选，让更有用的页靠前。", example: "[A,B,C] 变成 [B,A,C]；漏掉的 F 无法进入这个列表。"},
      {term: "候选编号", en: "Local Candidate Index", definition: "页面在本次输入图片列表中从 1 开始的位置。", example: "输出中的 3 指第 3 张输入图，不一定是 PDF 第 3 页。"},
      {term: "相关页标签", en: "Gold / Positive Pages", definition: "数据集中标为相关的全部页面，包括检索没找到的页。", example: "gold={B,F} 时，即使 F 未进入候选，Recall 的分母仍是 2。"},
      {term: "输入", en: "Prompt", definition: "模型生成前看到的指令、问题和内容。", example: "本项目是问题、五张页面图，以及输出格式要求。"},
      {term: "目标输出", en: "Completion", definition: "希望模型学会生成的助手回答。", example: "本项目是简短证据说明，加上全部候选编号的排列。"},
      {term: "Token", en: "Token", definition: "模型处理序列的单位；文本 token 不一定是一个完整词。", example: "训练时逐个预测目标回答的下一个 token。"},
      {term: "SFT", en: "Supervised Fine-Tuning", definition: "监督微调：用输入和目标输出的示范，继续训练已有模型。", example: "这次已保存实验使用 7,200 条证据与排序示范。"},
      {term: "损失", en: "Loss", definition: "衡量预测与目标相差多少，训练尝试降低它。", example: "正确的下一个 token 概率越高，对应的交叉熵损失越小。"},
      {term: "LoRA", en: "Low-Rank Adaptation", definition: "冻结基础模型的大权重矩阵，只训练附加的小矩阵。", example: "本次 SFT 使用 rank 8、alpha 32。"},
      {term: "GRPO", en: "Group Relative Policy Optimization", definition: "同一道题生成多份输出，比较奖励后更新模型。", example: "提高组内表现较好输出的生成概率。"},
      {term: "奖励", en: "Reward", definition: "训练程序为输出计算的分数，用来表达偏好。", example: "可以奖励格式和排序；它未必检查解释是否符合事实。"},
      {term: "优势", en: "Advantage", definition: "一份输出的奖励相对组内平均值有多好，通常还会归一化。", example: "全组奖励相同时，相对奖励优势都是 0。"},
      {term: "Recall@K", en: "Recall at K", definition: "前 K 位命中的相关页数，除以全部相关页数。", example: "总共 2 张相关页，第一位命中 1 张：Recall@1=1/2。"},
      {term: "格式回退", en: "Fallback", definition: "模型输出不符合完整排列规则时，继续使用原检索顺序。", example: "[2,2,3] 有重复编号，不是三张页面的合法排列。"}
    ]
  }
};
