...
(() => {
  ...
  const chapters = [
    ...
    ['results','比较训练前后','Compare the results','不做 SFT，会怎样？','What happens without SFT?'],
    ['grpo','GRPO 基础','GRPO basics','接下来，用奖励比较输出。','Next, compare outputs with rewards.'],
  ...
  const modelName = id => ({retrieval:t('ColQwen2 原检索','ColQwen2 retrieval'),base:t('Base · 未做 SFT','Base · no task SFT'),sft:t('SFT 后的 Qwen','Qwen after SFT')})[id];
  ...
  function sft() {
    ...
    return L[lang].sft.replace('<h3>4 ·',lossPanel+'<h3>4 ·').replace('<h3>5 ·',loraLab()+'<h3>5 ·') + snippet('sftTrain');
  }
  function results() {
    return `<p class="chapter-intro">${t('把同一批测试候选交给三种方法，比较最终排序。Base 是没有做本项目 SFT 的原始 Qwen。','Compare three methods on the same test candidates. Base is the original Qwen without this project’s SFT.')}</p>
    ${G[lang].resultsIntro}
    <div class="result-summary"><div><strong>1,658</strong><span>${t('独立测试问题','held-out test questions')}</span></div><div><strong>+3.09 pp</strong><span>${t('SFT − Base · Recall@1','SFT − Base · Recall@1')}</span></div></div>
    <label for="metric-select">${t('指标','Metric')}</label> <select id="metric-select">${[['recall1','Macro Recall@1'],['recall3','Macro Recall@3'],['recall5','Macro Recall@5'],['mrr','MRR'],['ndcg5','nDCG@5']].map(([key,name])=>`<option value="${key}" ${state.metric===key?'selected':''}>${name}</option>`).join('')}</select><div id="metric-chart" class="metric-bars"></div>
    <p class="muted" id="metric-description"></p><p class="takeaway">${t('Base 有 <strong>1,607 / 1,658</strong> 个输出不符合格式要求，SFT 后只有 <strong>1 / 1,658</strong>。失败时使用原检索顺序计分，因此提升包含“学会输出格式”的贡献。','Base has <strong>1,607 / 1,658</strong> format failures; after SFT there is <strong>1 / 1,658</strong>. Failed outputs fall back to retrieval order, so the improvement includes learning the output format.')}</p>
    ${G[lang].resultsAfter}
    ${details('replay-details',t('展开真实案例：训练前后输出','Explore real cases: before and after SFT'),`<div class="case-controls"><label for="case-select">${t('案例','Case')}</label><select id="case-select">${D.cases.map((c,i)=>`<option value="${i}" ${state.caseIndex===i?'selected':''}>${esc(caseInfo(c).title)}</option>`).join('')}</select><button id="toggle-gold"></button></div><div id="case-body"></div>`)}
    ...
    ${details('evaluation-notes',t('评估口径与来源','Evaluation protocol and sources'),`<p>${t('SFT−Base 的 Macro Recall@1 配对 95% 区间为 +1.28 至 +4.93 个百分点。每题先计算 Recall 再取平均；1,000 次按查询重采样，未按文档聚类，也不包含重新训练的随机种子方差。','The paired 95% interval for SFT−Base Macro Recall@1 is +1.28 to +4.93 percentage points. Recall is computed per query and averaged; the interval uses 1,000 query bootstrap samples, without document clustering or retraining-seed variance.')}</p><p>${t('训练数据用于更新参数，验证数据用于选检查点，测试用于最终比较。模型使用相同候选、提示和生成设置；保留所有失败题，Recall 分母包括未召回的相关页。固定 top-5 集合的 Recall@5 不变。','Training updates parameters, validation selects checkpoints, and testing measures the final comparison. Models share candidates, prompts and generation settings. Failed queries remain in the evaluation; missed relevant pages remain in the Recall denominator. Recall@5 is unchanged for a fixed top-5 set.')}</p><p>${t('排序得分不能证明解释正确；少量案例也不能替代整体成绩。这里展示上一轮已完成的固定 Base/SFT 快照。','Ranking scores do not establish explanation accuracy, and selected examples do not replace aggregate results. This page uses the saved, completed Base/SFT snapshot.')}</p>`)}
    <p class="next-idea">${t('接下来：SFT 之后，还可以尝试让模型从奖励中学习。','Next: after SFT, another option is learning from rewards.')}</p>`;
  }
  ...
  function grpo() {
    return L[lang].grpo + details('reward-lab',t('手算一组奖励与相对优势','Calculate rewards and relative advantages'),`<p>${t('假设 5 张候选中 {2, 4} 相关。同一道题生成两份输出，比较奖励：','Suppose images {2, 4} are relevant among five candidates. Compare two outputs for the same question:')}</p><div class="reward-controls"><div><label for="reward-a">${t('输出 A','Output A')}</label><select id="reward-a">${rewardOptions(state.rewardA)}</select></div><div><label for="reward-b">${t('输出 B','Output B')}</label><select id="reward-b">${rewardOptions(state.rewardB)}</select></div></div><div id="reward-result" aria-live="polite"></div><p class="muted">${t('人工示意：合法格式奖励 1，加上相关页名次的 1/r³ 得分，并用理想顺序归一化；非法排列两项均为 0。优势用组内总体标准差 + 10⁻⁸ 归一化。不是当前实验参数或训练日志。','Illustration: valid format earns 1, plus a ranking score summing 1/r³ for relevant-page ranks and normalized by the ideal order. Invalid permutations earn zero for both. Advantages use the population group standard deviation + 10⁻⁸. These are not current experiment settings or training logs.')}</p>`).replace('<details ', '<details open ');
  }
  function resources() {
    const docs=[['base-evaluation','Base 评估报告','Base evaluation report'],['sft-evaluation','SFT 评估报告','SFT evaluation report']];
    ...
    ${details('reference-files',t('项目报告与来源记录','Project reports and provenance'),`<p class="muted">${t('这里提供本项目的数据、训练与评估资料；中英文课堂使用同一份数据和结果。','These project files document data, training and evaluation. Both classroom languages use the same data and results.')}</p><div class="resource-list">${docs.map(([id,zh,en])=>`<a href="assets/sources/docs__${id}.md.txt" target="_blank" class="resource-item">${t(zh,en)}${lang==='en'?' · original Chinese':''}</a>`).join('')}<a href="assets/provenance.json" target="_blank" class="resource-item">${t('数据与源码来源清单','Dataset and source provenance')}</a></div>`)}
